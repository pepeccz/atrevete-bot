...
**Rewrote**: 2026-03-27 — prior spec targeted non-existent FSM functions
...
| Item | Evidence |
|------|----------|
| `first_name` NOT injected in `build_step_context` | `loader.py:295`, comment "Customer name is intentionally NOT injected" |
| `build_step_context` filters `first_name` | `test_prompt_loader.py:789–830` (T3.6) |

---

## GAP 1 — Inline customer creation for new WhatsApp users

**File**: `agent/modes/booking_mode.py`
**Location**: `_pre_tool_call`, after line 709 (before the `NO_CUSTOMER_ID` rejection)
...
## GAP 2 — `last_error` not set in `mode_context` on booking failure
...
## GAP 3 — `error_count` never incremented

**File**: `agent/modes/booking_mode.py`
**Location**: `_build_response`, after the booking result check

`conversation_flow.py:408` reads `error_count` for auto-escalation (`>= 3`), but nothing increments it. When `book()` returns a failure (not `ctx._booking_completed`), `_build_response` MUST add `"error_count": state.get("error_count", 0) + 1` to the returned state update dict. On success it MUST NOT change `error_count`.
...
## GAP 4 — Test T3.7 references non-existent `booking/confirmation.md`

**File**: `tests/unit/test_prompt_loader.py`
**Location**: `TestConfirmationMdNameLine` class, lines 836–868

The directory `agent/prompts/modes/booking/` does not exist. The actual prompt is `agent/prompts/modes/booking.md`. The name-privacy concern the test addresses is now enforced by `_redact_names` at `booking_mode.py:1683`. The test MUST be redirected: it SHALL read `agent/prompts/modes/booking.md` (not `booking/confirmation.md`) and assert that it does NOT contain the string `"Puedes usar el nombre de la clienta"`. The second scenario MUST assert that `booking.md` contains core booking instructions.
...
- GIVEN the test class `TestConfirmationMdNameLine` is run
- WHEN it executes
...
## GAP 5 — No integration test for new WhatsApp user booking

**File**: `tests/integration/` (new file)
**Location**: New test class in `tests/integration/test_booking_mode_integration.py` or equivalent

There is no test that exercises `BookingMode.handle()` end-to-end with `pending_whatsapp_name` set and `customer_id=None`. One integration test MUST cover this scenario.
...
- Any prompt content changes beyond the test redirection in GAP 4
