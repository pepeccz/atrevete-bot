...
class TestBuildCatalogForPrompt:
    ...
    def test_variant_line_format(self):
        ...
        assert "[VARIANTE de Corte Señora · cut]" in result
...
class TestAsyncHelpers:
    ...
    async def test_load_active_catalog_returns_list(self):
        ...
        fake_services = [PRINCIPAL_CORTE_SENORA, PRINCIPAL_MANICURA]

        with patch(
            "agent.prompts.catalog_builder._query_active_services",
            new=AsyncMock(return_value=fake_services),
        ):
    ...
    async def test_build_catalog_prompt_section_returns_str(self):
        ...
        fake_services = [PRINCIPAL_CORTE_SENORA]
