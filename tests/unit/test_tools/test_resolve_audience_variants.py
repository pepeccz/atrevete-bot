...
async def audience_services(db_session):
    """Seed two PRINCIPAL services sharing dimension='cut_test' with distinct audiences."""
    ...
    for name, aud in zip(names, ["adult_female", "adult_male"]):
        svc = Service(
            id=uuid4(),
            name=name,
    ...
    yield {"dimension": dim, "names": names}
...
@pytest.mark.asyncio
async def test_principals_same_dimension_differing_audience_returns_audience_kind(
    db_session, audience_services
):
    """H1: PRINCIPAL peers sharing dimension but different audience → kind=='audience'."""
    if not await _db_available():
        pytest.skip("Postgres not reachable")
    ...
    kind, family, candidates = await _resolve_audience_variants(
        db_session, audience_services["names"][0]
    )
