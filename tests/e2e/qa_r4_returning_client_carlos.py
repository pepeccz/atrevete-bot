"""QA Round 4 — returning_client flow with carlos_returning_client persona.

Validates:
  BUG-001: book() called on confirmation → appointment_in_db=true
  BUG-003: Must NOT ask "¿caballero, dama, niño?" — "corte caballero" already implies variant
  BUG-006: Greeting once only on Turn 1
  NEW-B:   Add-on implicit decline works (ambiguous reply = skip add-on)
  NEW-REG-2 (slot loop): "esta semana a la mañana" must be accepted as valid date range
"""
...
# Persona: Carlos — returning client, QA Round 4
PERSONA_NAME = "Carlos"
CUSTOMER_PHONE = "+34622334455"  # Fresh phone — never reuse conversation_id

# Track how many times we've seen "esta semana" loop (NEW-REG-2)
date_repeat_count = 0
...
def decide_next_reply(turn_num: int, bot_message: str, history: list) -> tuple[str, bool]:
    ...
    if any(kw in msg_lower for kw in [
        ...
        "¿querés confirmar", "¿te parece bien",
    ]):
        return ("Sí, confirmo", False)

    # Bot offers add-ons (NEW-B: ambiguous reply to test implicit decline)
    if any(kw in msg_lower for kw in [
        "barba", "cejas", "masaje", "tratamiento", "¿querés agregar",
        "¿te gustaría agregar", "add-on", "adicional", "también podés",
        "¿sumás", "¿agregás",
    ]):
        # Use an ambiguous/neutral reply to test implicit decline (NEW-B)
        return ("No, gracias", False)

    # Bot asks about time/day — this is NEW-REG-2 scenario
    if any(kw in msg_lower for kw in [
    ...
    if any(kw in msg_lower for kw in [
        ...
        "tipo de corte", "dama o caballero",
    ]):
...
async def run_qa():
    print(f"\n{'='*70}")
    print("QA ROUND 4 — returning_client — carlos_returning_client")
    print(f"{'='*70}\n")
