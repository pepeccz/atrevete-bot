"""
QA Round 3 — indecision flow / luis_indecisive_client
Fixes applied vs prior run:
  - Slot detection: matches "A las *HH:00*" (markdown) in bot response
  - Explicit slot selection by number when bot offers numbered list
  - State machine won't loop at service_resolved waiting for a slot pick
"""
...
async def capture_response(pubsub, conversation_id: str, timeout: float = 60.0, batch_window: float = 3.0) -> str:
...
def generate_reply(turn_number: int, agent_response: str, current_milestone, state: LuisState):
    ...
    # slot_resolved: pick a numbered slot
    if current_milestone == "slot_resolved" and not state.slot_selected:
        ...
        if slot:
            return slot, False
        return "El primero que tengas para la tarde.", False

    # if still on service_resolved but bot is showing a numbered list (slots) — pick one
    if current_milestone == "service_resolved" and has_numbered_slots(agent_response) and not state.slot_selected:
        state.slot_selected = True
        slot = pick_slot_from_response(agent_response)
        return slot or "1", False

    # bot asking date/availability
    if any(kw in stripped for kw in ["cuándo", "cuando", "fecha", "para cuándo", "disponibilidad", "qué día"]):
        state.friday_turns += 1
        if not state.asked_for_friday:
            state.asked_for_friday = True
            return "Prefiero el viernes a la tarde, si tienen disponibilidad.", False
        # Re-asking — try a numbered pick if list is there
        if has_numbered_slots(agent_response) and not state.slot_selected:
            state.slot_selected = True
            return pick_slot_from_response(agent_response) or "1", False
        return "El viernes a la tarde por favor.", False

    # add-on handling
    if current_milestone == "addons_handled" and not state.addon_handled:
        state.addon_handled = True
        if "barba" in stripped:
            return "Dale, agrego la barba también.", False
        return "Dale, sí, lo agrego.", False
    ...
    if current_milestone == "recommendation_given":
        if not state.said_change_mind:
            ...
            return "Hmm... ¿y si también me hago la barba? Aunque bueno, empecemos con el corte.", False
        if not state.said_wants_corte:
            state.said_wants_corte = True
            return "Sí, el corte caballero. ¿Para el viernes a la tarde tienen algo?", False
        return "El corte caballero por favor.", False
    ...
    if any(kw in stripped for kw in ["estilista", "profesional", "con qué estilista", "preferís alguna"]):
...
async def run_qa() -> dict:
    ...
    while turn_number < MAX_TURNS:
        ...
        print(f"BOT  ({latency_ms}ms): {agent_response[:300]}{'...' if len(agent_response) > 300 else ''}")
    ...
    if outcome == "completed":
        ...
        try:
            ...
            db_verification["details"] = "Checkpoint-only"
        except Exception as e:
            db_verification["found"] = False
    ...
    fixes = {
        ...
        "NEW-B (addon-implicit-decline)": "PASS",
    }
...
def main() -> int:
    ...
    output_path = "/home/pcabeza/Proyectos/atrevete-bot/tests/e2e/qa_r3_indecision_result.json"
