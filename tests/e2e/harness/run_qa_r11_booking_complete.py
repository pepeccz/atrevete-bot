"""QA Round 11 — booking_complete flow — maria_new_client persona.

Key fixes in 55ab710:
  FIX-1: selected_slot written as canonical DTO with start_time (not full_datetime mismatch)
  FIX-2: offered_slots always refreshed (not guarded by 'if not exists')
  FIX-3: offered_slots/selected_slot cleared on stylist/service rewinds
  FIX-4: booking_payload_ready() guard before book() — no phantom confirmations
  FIX-5: _handle_completed() fail-closed — customer_id required before book()
  FIX-6: normalize_slot_entry() ensures full_datetime always present

Harness script per spec:
  T1: "Hola, quiero reservar un corte de cabello para dama"
  (variant ask dama/niña/bebé): "Dama"
  (service list shown): "El primero"
  ...
  (error recovery): "Intenta de nuevo"
...
PHONE = "+34600000012"  # fresh phone for R11
MAX_TURNS = 20
...
def pick_response(text: str, turn_num: int) -> str | None:
    ...
    R11 key difference from R10:
    - commit 55ab710: slot DTO is now canonical, offered_slots always refreshed
    - FSM should now correctly proceed to CUSTOMER_NAME after slot pick
    - All 6 FSM invariant fixes are deployed
    """
    t = text.lower()
...
async def run_qa():
    ...
    print(f"\n{'='*60}")
    print(f"QA Round 11 — booking_complete flow")
    print(f"Persona: maria_new_client ({PERSONA_NAME})")
    print(f"Conversation ID: {conversation_id}")
    print(f"Started: {datetime.now(UTC).isoformat()}")
    print(f"Commit: 55ab710")
    print(f"{'='*60}\n")
...
async def get_agent_logs(conversation_id: str) -> list[str]:
    ...
    relevant = [l for l in lines if conversation_id[:8] in l
                ...
                or "SLOT_SELECTION" in l
                or "intent=" in l
                or "substep" in l.lower()
                or "booking_payload_ready" in l
                or "offered_slots" in l.lower()
                or "selected_slot" in l.lower()
                or "normalize_slot" in l.lower()
                or "full_datetime" in l.lower()
                or "FIX-" in l
