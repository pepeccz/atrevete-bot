"""Google Calendar service — canonical implementation for admin-panel use.

This module is the canonical home for Google Calendar integration consumed by
api/routes/admin.py. It was relocated from agent/tools/calendar_tools.py, which
retains a thin backward-compat reverse shim.

Architecture note: imports `agent.services.gcal_credential_factory` lazily
(inside CalendarTools.get_service()). This is an intentional exception:
credential resolution depends on google_oauth_service and DB models that live
in agent/. Moving the factory would cascade into a larger surface — that
migration is a follow-up (see ADR sdd/agent-tools-shared-extraction).

The rule being enforced by this change is: `api → shared, not api → agent`.
The `shared → agent.services` transitive dependency is acceptable as a
pragmatic exception, documented here explicitly.

Agent graph does NOT use this module for availability — it uses
agent/services/availability_service.py (DB-first, <100ms).
This module is strictly an admin-panel helper for displaying Google Calendar
events alongside DB appointments on the calendar view.
"""
...
class CalendarTools:
    ...
    async def get_service(self) -> Any:
        ...
        Architecture exception: imports gcal_credential_factory lazily from
        agent.services — see module docstring for rationale.
        """
