...
class ConversationHistory(Base):
    ...
    # PR-2: operator notes for this conversation
...
class ConversationMessage(Base):
    ...
    # PR-3a — attachments auto-loaded alongside the message row
...
# ============================================================================
# PR-2 — ConversationNote
# ============================================================================


class ConversationNote(Base):
    """
    Operator notes attached to a specific conversation.

    Notes are written by human agents (admin_users) to record context
    about a customer interaction — allergies, preferences, follow-up tasks.

    ON DELETE CASCADE from conversation_history ensures notes are removed
    when a conversation is deleted.  ON DELETE SET NULL from admin_users
    preserves notes when an admin account is deleted (author_name falls back
    to "Cuenta eliminada" in the UI).
    """
    ...
    # Note body — trimmed at service layer; never empty
    content: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    # Indexes
    __table_args__ = (
        # Primary query: notes for a conversation ordered by recency
        Index(
            "idx_conversation_notes_conv",
            "conversation_history_id",
            "created_at",
        ),
        Index("idx_conversation_notes_author", "author_user_id"),
    )

    def __repr__(self) -> str:
        return (
            f"<ConversationNote(id={self.id}, "
            f"conversation_history_id={self.conversation_history_id})>"
        )


# ============================================================================
# PR-3a — MessageAttachment
# ============================================================================


class MessageAttachment(Base):
    """
    File/media attachment linked to a single ConversationMessage.

    Rows are inserted by the Chatwoot webhook handler after the parent
    ConversationMessage is flushed.  The ``file_type`` column uses the string
    value provided directly by Chatwoot (e.g. "image", "audio", "file") — no
    integer mapping needed.

    ON DELETE CASCADE from conversation_messages ensures attachments are
    removed when the parent message is deleted.
    """
    ...
    # Attachment kind — Chatwoot string value: "image" | "audio" | "video" | "file" | etc.
    file_type: Mapped[str] = mapped_column(Text, nullable=False)

    # Storage URL (Chatwoot data_url — Rails ActiveStorage redirect link)
    ...
    # Relationship back to parent message
    message: Mapped["ConversationMessage"] = relationship(
        "ConversationMessage",
        back_populates="attachments",
    )

    # Composite index — ordered attachment retrieval per message
    __table_args__ = (
        Index(
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"file_type='{self.file_type}', position={self.position})>"
        )
