...
### ADR-006: FSM Híbrida para Control de Flujo (2025-11-21)

**Contexto:** La arquitectura LLM-driven v3.2 produce bugs sistemáticos porque el LLM controla flujo además de NLU. Durante Epic 1 Story 1-5 se descubrieron bugs críticos: state flags nunca se actualizaban, el agente no progresaba correctamente entre estados del booking flow.

**Decisión:** Implementar FSM híbrida donde LLM solo maneja NLU y generación de lenguaje, mientras FSM controla flujo de conversación.
...
- Bugs de Epic 1 Story 1-5 resueltos

**Consecuencias:**
- Nueva carpeta `agent/fsm/` con BookingFSM, IntentExtractor
- Refactoring de `conversational_agent.py` para integrar FSM
- Tools refactorizadas para FSM validation
- Tests más fáciles de escribir y mantener

**Referencias:**
- Sprint Change Proposal: `docs/sprint-change-proposal-2025-11-21.md`
- Epic 5 Spec: `docs/epics/epic-5-rediseño-fsm-hibrida.md`

### ADR-007: FSM TTL Synchronization with Checkpoint TTL (2025-11-22)

**Contexto:** Se identificó un bug de desincronización entre dos sistemas de persistencia paralelos:
- LangGraph Checkpoint (AsyncRedisSaver): TTL 24 horas
- BookingFSM (Redis separado): TTL 15 minutos (original)

Cuando el FSM expiraba antes que el checkpoint, el bot "recordaba" la conversación pero "olvidaba" el booking en progreso, causando respuestas incoherentes.

**Decisión:** Sincronizar FSM TTL con Checkpoint TTL a 24 horas.

**Cambio:**
```python
# agent/fsm/booking_fsm.py:26
FSM_TTL_SECONDS: int = 86400  # Era 900 (15min), ahora 24h
```

**Razones:**
- Elimina 100% de bugs de desincronización por diferencia de TTL
- Usuarios pueden continuar bookings después de pausas de hasta 24h
- Sin cambios en lógica de negocio

**Consecuencias:**
- (+) Experiencia consistente para usuarios que regresan
- (+) Eliminación de mensajes incoherentes ("Hola" cuando usuario está en booking)
- (-) Ligeramente más memoria Redis (negligible, ~200 bytes por conversación)

**Mejora Futura Considerada:**
Unificar FSM state en ConversationState (eliminar segundo sistema de persistencia). Diferido a post-Epic 5 por esfuerzo de refactoring.

**Referencias:**
- Sprint Change Proposal: `docs/sprint-change-proposal-2025-11-22-fsm-ttl-fix.md`

---

_Generated by BMAD Decision Architecture Workflow v1.0_
_Date: 2025-11-19_
_Updated: 2025-11-22 (ADR-007 added)_
_For: Pepe_
