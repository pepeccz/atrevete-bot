...
9b. **Desambiguación de variantes**: antes de resolver un `service_id`, revisa si en el catálogo existen OTRAS entradas tagueadas como `[VARIANTE de X]` donde X coincide con el nombre genérico dicho por el cliente (ej. cliente dice "peinado" → catálogo tiene `Peinado [PRINCIPAL]`, `Peinado Largo [VARIANTE de Peinado]`, `Moldeado Extra [VARIANTE de Peinado]`). En ese caso PREGUNTA una vez al cliente qué variante quiere (incluyendo la principal) antes de llamar a cualquier herramienta. No preguntes si el cliente ya nombró la variante exacta (ej. "peinado largo", "óleo pigmento" no requieren desambiguación) ni si el servicio no tiene variantes hijas en el catálogo.
