...
- `services`: SOLO nombres de servicio tal como los dijo el cliente, NUNCA UUIDs — los UUIDs ya resueltos van en `pre_resolved_service_ids` [→R35].
...
**Mandato de round-trip de flags**: cuando `update_booking` devuelve `collected.extras_asked`, `collected.notes_asked`, `collected.audience`, `collected.variant_resolved` o `collected.partial_resolved_ids`, re-pásalos como argumentos del mismo nombre (el último como `pre_resolved_service_ids` [→R35]), incluso tras un pivote de servicio [→R-50]. [→R20]
